...
class STDHMP_D(nn.Module):
    ...
    def forward(self, data_batch, loc_tar, mode='train'):
        ...
        if self.type_loc_embedder == 'gnn':
            (uid_batch, attr_batch, loc_src, mask_day, mask_traj), graph = data_batch
        else:
            # attr_batch=3*(B,1), loc_src=(B,7,S=26) with token, tim_src=(B,7,S=24), mask_day=(B, 7), mask_traj=(B,7,S=26), loc_tar=(B,S)
            uid_batch, attr_batch, loc_src, mask_day, mask_traj = data_batch
        ...
        # decode trajectory
        # train mode
        loc_vector = torch.zeros((loc_tar.size()[0], loc_tar.size()[1], 10))
        if mode == 'train':
            if self.type_loc_embedder == 'gnn':
        ...
        # eval mode
            ...
            for i in range(loc_chain.shape[1]-1):
                ...
                if self.beam_size > 1:
                    ...
                    _,loc_top10_indices=torch.topk(loc_pred, k=10, dim=-1)
                    for b in range(loc_vector.size()[0]):
                        for j in range(loc_vector.size()[-1]):
                            loc_vector[b][i+1][j]=loc_top10_indices[b][i][j]
                    loc_next = torch.argmax(loc_pred, dim=-1)   # (B, S)
                    loc_chain_reshaped[:, i+1] = loc_next[:, i]  # (B, S)
            loc_chain = loc_chain_reshaped.reshape(loc_chain.shape[0], -1, loc_chain.shape[-1])[:, 0, :]  # (B', S=26) -> (B, S=24)

            return loc_chain,loc_vector
